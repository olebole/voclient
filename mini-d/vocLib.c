...
voc_initVOClient (char *opts)
{
    ...
    */
	...
	vopt = (vocOpt *) calloc (1, sizeof (vocOpt));
        ...
        vopt->use_runid      = TRUE;
        vopt->console        = FALSE;
	vopt->use_mini_d     = TRUE;
	vopt->handle_context = -1;
    ...
    */
    ...
    } else if (vopt->use_mini_d || getenv ("VOC_STANDALONE")) {
        /* Allocate the struct.
	*/
	if (getenv ("VOC_DEBUG"))
	    fprintf (stderr, "Using mini-daemon .....\n");
	...
	*/
	...
	if (getenv ("VOC_DEBUG"))
	    fprintf (stderr, "Using Java daemon .....\n");
	...
	*/
    ...
    vo->use_mini_d     = vopt->use_mini_d;
    if (vopt->handle_context < 0)
        vo->handle_context = svr_newHandleContext ("_VOCLIB_", 8192);
    else
        vo->handle_context = vopt->handle_context;
...
voc_closeVOClient (int shutdown)
{
	 ...
	 * so we don't block.
	 */
	if ( !vo->use_mini_d  &&  !getenv ("VOC_STANDALONE") )
    ...
    if (vo->use_mini_d == TRUE)
	svr_closeHandleContext (vo->handle_context);
...
voc_parseOpts (char *opts)
{
	...
	*/
	...
	else if ( (strncmp ("inline", keyw, 6) == 0) ||
		  (strncmp ("standalone", keyw, 6) == 0) ||
		  (strncmp ("mini-d", keyw, 6) == 0) )
	    msg_setMiniDaemon ( (lmini_d = 1) );

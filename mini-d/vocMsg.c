...
/* MSG_SETMINIDAEMON -- Enable/Disable messaging to use mini daemon.
 */
int
msg_setMiniDaemon (int val)
{
    use_mini_daemon = val;
    return (val);
}
...
msg_sendMsg (int fd, vocMsg_t *msg)
{
    ...
    if (use_mini_daemon) {
        return ( svr_processMsg (msg) );

    } else {
	stat = msg_sendRawMsg (fd, msg);
...
msg_sendRawMsg (int fd, vocMsg_t *msg)
{
	...
	/*  Ensure a newline on the message.
	 */

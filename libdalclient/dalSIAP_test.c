/**
 * DALSIAP_TEST -- Test the SIAP service interface.
 *
 * This test program exercises and demonstrates the DALClient SIAP service
 * interface, and most of the full DALClient library in the process.  Only
 * the standard client API is used here (no use of internals).
 *
 * @file	dalSIAP_test.c
 * @author	Doug Tody
...
#define	LOC_SIAPV2	"http://localhost:8080/ivoa-dal/siapv2"

/* SIAV2 reference service running on the a NRAO VAO server. */
#define	VAO_SIAPV2	"http://vaosa-vm1.aoc.nrao.edu/ivoa-dal/siapv2"
...
main (int argc, char *argv[])
{
     ...
     */
    if (band)
	dal_addStringParam (query, "BAND", band);
    if (time)
	dal_addStringParam (query, "TIME", time);
    if (pol)
	dal_addStringParam (query, "POL", pol);

    /* Dummy param to test parameter deletion. */
    dal_addStringParam (query, "foo", "bar");

    /* Type is "image" (2D) or "cube" (>= 3D); default all nD images. */
    if (imtype)
	dal_addStringParam (query, "TYPE", imtype);

    if (mode)
	dal_addStringParam (query, "MODE", mode);         /* no cutouts yet */
    if (collection)
	dal_addStringParam (query, "COLLECTION", collection);

    /* Maximum number of table rows in query response. */
    if (maxrec)
	dal_addStringParam (query, "MAXREC", maxrec);
    ...
    if (!timeSet && updParam) {
	dal_setParam (query, "time", "1990-07-04/2014");  /* ISO 8601 range */

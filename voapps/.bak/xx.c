...
/**
 *  VOT_IMAGEINFO -- Get informat about a FITS file structure.
 */
ImInfo *
vot_imageInfo (char *name, int do_all)
{
    ...
    info->frame.xrpix  = info->frame.radius / (info->frame.scale / 3600.) / 2.0;
    info->frame.yrpix  = info->frame.radius / (info->frame.scale / 3600.) / 2.0;
...
/**
 *  VOC_FREEIMAGEINFO -- Print the image information.
 */
void
vot_freeImageInfo (ImInfo *img)
{
    free ((void *) img->extns);            	/* extension structs    */
    free ((void *) img);                        /* main image structs   */
}
